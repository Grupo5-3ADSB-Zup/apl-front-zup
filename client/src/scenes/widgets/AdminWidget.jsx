...
const AdminWidget = () => {
  ...
  const handleImportFile = async () => {
    ...
    input.onchange = async (event) => {
      ...
      try {
        await axios.post('http://44.219.155.152:8080/admin/importacao/txt', formData);
  ...
  const getUser = async () => {
    try {
      const response = await axios.get("http://44.219.155.152:8080/admin");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://44.219.155.152:8080/usuario/${id}`);
  ...
  const handleGenerateFile = async () => {
    ...
    try {
      const response = await axios.get(`http://44.219.155.152:8080/admin/csv/${fileName}`, {
        filename: fileName,
      }, {
        responseType: 'blob',
      });
  ...
  const handleSwitchChange = async (id, isActive) => {
    try {
      const response = await axios.put(`http://44.219.155.152:8080/admin/influencer/comum/${id}/${!isActive}`);
